...
import { Search, ShieldAlert, Sliders, Save } from "lucide-react";
...
export function FlagsPage() {
  ...
  // Risk Rules Configuration state
  const [threshold, setThreshold] = useState(() => {
    const saved = localStorage.getItem(`high_risk_threshold_${organization?.slug}`);
  ...
  const handleSaveRiskRules = () => {
    setSavingRules(true);
    try {
      localStorage.setItem(`high_risk_threshold_${organization?.slug}`, threshold.toString());
      localStorage.setItem(`high_risk_auto_escalate_${organization?.slug}`, String(autoEscalate));
      localStorage.setItem(`high_risk_parent_notify_${organization?.slug}`, String(parentNotify));
      toast.success(`Risk rules updated: High-risk threshold set to ${threshold} incidents`);
    } catch {
      toast.error("Failed to save risk rules");
    } finally {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-150">
      <PageHeader
        eyebrow="Operations"
        title={isStaff ? "Discipline & Incident Reports" : "My Disciplinary Records"}
        description={
          isStaff
            ? "Student incident flags, disciplinary records, automated risk rules, and Warden resolution tracking"
            : "Review any active or resolved disciplinary incident reports filed for your account"
        }
        breadcrumbs={[
      ...
      {isStaff && (
        <div className="flex items-center gap-2 border-b border-[var(--color-border)] pb-3">
          ...
          >
            Incidents & Records ({flags.length})
          </button>
          <button
            type="button"
      ...
      {/* TAB 2: RISK RULES & THRESHOLDS */}
      {activeTab === "rules" && (
        <div className="max-w-3xl space-y-6">
          <Card className="p-6 bg-[var(--color-surface)] border border-[var(--color-border)] shadow-xs space-y-6">
            <div className="border-b border-[var(--color-border)] pb-4">
              <div className="flex items-center gap-2">
                <ShieldAlert className="h-5 w-5 text-[var(--tenant-primary)]" />
                <h3 className="font-h3 text-base text-[var(--text-primary)] font-semibold">
                  Disciplinary Risk Rules & Thresholds
                </h3>
              </div>
              <p className="text-xs text-[var(--text-muted)] mt-1">
                Configure automated risk monitoring rules, escalation thresholds, and notifications for chronic student disciplinary infractions.
              </p>
            </div>
              ...
              <div className="p-4 rounded-xl bg-[var(--color-surface-sunken)] border border-[var(--color-border)] space-y-3">
                <div>
                  <h4 className="text-xs font-bold text-[var(--text-primary)] uppercase tracking-wider">
                    High-Risk Student Threshold
                  </h4>
                  <p className="text-xs text-[var(--text-muted)] mt-0.5">
                    Students with this number or more of active, unresolved incidents are marked with a High-Risk warning badge across the directory and gate scanner.
                  </p>
                </div>
                ...
                <div className="flex items-center gap-3 pt-1">
                  <div className="w-32">
                    ...
                    <input
                      ...
                      className="w-full h-9 rounded-md bg-[var(--color-surface)] border border-[var(--color-border)] px-3 text-xs text-[var(--text-primary)] font-mono focus-visible:outline-none focus-visible:border-[var(--tenant-primary)]"
                    />
                  </div>
                  <div className="text-xs text-[var(--text-muted)] self-end pb-2">
                    incidents &ge; <span className="font-bold text-[var(--color-danger)]">{threshold}</span> &rarr; High Risk Triggered
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-2 pt-1 text-xs">
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    ...
                    <span className="text-[var(--text-primary)]">
                      Notify hostel administrator and chief warden upon risk breach
                    </span>
                  </label>
                  ...
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    ...
                    />
                    <span className="text-[var(--text-primary)]">
                      Enable guardian SMS/WhatsApp notification alert trigger
                    </span>
                  </label>
                </div>
              </div>
            ...
            <div className="flex justify-end pt-2 border-t border-[var(--color-border)]">
              ...
              >
                ...
                {savingRules ? "Saving Rules..." : "Save Discipline Rules"}
              </Button>
            </div>
          </Card>
        </div>
